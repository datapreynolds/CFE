...
train_cropped <- train %>% mutate(DTI = replace(DTI, DTI > 2.5 | is.na(DTI), 2.5),
                                  LTV = replace(LTV, LTV > 2.5 | is.na(LTV), 2.5),
                                  Lender = Source == 'LENDER',
                                  Consumer = Source == 'CONSUMER',
                                  BadCredit = ModifiedCreditScore >= 400 & ModifiedCreditScore < 650,
  ...
  filter(Loanterm * EstimatedMonthlyPayment != AmountRequested)
```

```{r train valid split}
sub_train <- train_cropped %>% sample_frac(.7)
