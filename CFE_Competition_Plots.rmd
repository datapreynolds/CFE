...
MCS_data <- train_cropped %>%
  mutate(ModifiedCreditScore = ModifiedCreditScore - (ModifiedCreditScore %% 2)) %>%
...
```{r accounts vs approval}
accounts_data <- train %>%
  group_by(NumberOfOpenRevolvingAccounts) %>%
  summarize(ApprovalRate = mean(LoanStatus == 'Approved'), Count = n(), AvgCreditScore = mean(ModifiedCreditScore))

ggplot(accounts_data, aes(NumberOfOpenRevolvingAccounts, ApprovalRate, size = Count)) + geom_point()
```

```{r accounts vs credit score}
accounts_data <- train %>%
  group_by(NumberOfOpenRevolvingAccounts) %>%
  summarize(ApprovalRate = mean(LoanStatus == 'Approved'), Count = n(), AvgCreditScore = mean(ModifiedCreditScore))

ggplot(accounts_data, aes(NumberOfOpenRevolvingAccounts, AvgCreditScore, size = Count)) + geom_point()
```